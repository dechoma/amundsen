...
from metadata_service.proxy.atlas_utils import AtlasColumnKey, Status, AtlasTableKey
...
class AtlasProxy(BaseProxy):
    ...
    def _get_table_entity(self, *, table_uri: str) -> AtlasEntityWithExtInfo:
        ...
        try:
            return self.client.entity.get_entity_by_attribute(type_name=key.get_details()['source'],
                                                              uniq_attributes=[(self.QN_KEY, key.qualified_name)])
    ...
    def get_table(self, *, table_uri: str) -> Table:
        ...
        try:
            ...
            table_info = AtlasTableKey(attrs.get(self.QN_KEY)).get_details()
    ...
    def _serialize_popular_tables(self, entities: list) -> List[PopularTable]:
        ...
        for table in entities:
            ...
            table_info = AtlasTableKey(table_attrs.get(self.QN_KEY)).get_details()

            table_name = table_info.get('table') or table_attrs.get('name')
            db_name = table_info.get('db', '')
    ...
    def _get_resources_followed_by_user(self, user_id: str, resource_type: str) \
        ...
        for record in search_results.entities:
            table_info = AtlasTableKey(record.attributes[self.ENTITY_URI_KEY]).get_details()

            res = self._parse_bookmark_qn(record.attributes[self.QN_KEY])
            resources.append(PopularTable(
                database=table_info['source'],
                cluster=res['cluster'],
                schema=res['db'],
                name=res['table']))
        return resources
        ...
        for reader in readers.entities or list():
            ...
            if count:
                ...
                _results[count] = dict(cluster=table_info.get('cluster'),
                                       ...
                                       schema=table_info.get('db'),
                                       database=table_info.get('source'))
    ...
    def _get_lineage_graph(self, lineage: Dict, entity_type: str, key_class: Any) -> Dict[str, List[str]]:
        ...
        for relation in relations:
            ...
            if input_type.endswith('process') and output_type.endswith(entity_type):
                output_qn = entities.get(output_guid)[self.ATTRS_KEY][self.QN_KEY]
                ...
                input_qn = entities.get(input_guid)[self.ATTRS_KEY][self.QN_KEY]
